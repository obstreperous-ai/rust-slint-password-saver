...
fn test_recovery_code_format() {
    ...
    assert_eq!(code.code.matches('-').count(), 3, "Recovery code should have 3 dashes");
...
fn test_save_and_load_with_recovery() {
    ...
    assert_eq!(
        loaded_hashes.len(),
        3,
        "Should have 3 recovery code hashes"
    );
}
...
fn test_recovery_codes_are_properly_hashed() {
    ...
    assert_eq!(codes.len(), hashes.len(), "Should have same number of codes and hashes");
